...
<table><tr><td bgcolor=gray><big>开放定址法</big></td></tr></table>

1. 思路:
   - 从发生冲突的那个单元起，按照一定的次序，从哈希表中找到一个空闲的单元。然后把发生冲突的元素存入到该单元的一种方法。
2. 实现:
   - **线行探查法**，它从发生冲突的单元起，依次判断下一个单元是否为空，当达到最后一个单元时，再从表首依次判断。直到碰到空闲的单元或者探查完全部单元为止。
   - **平方探查法**，用发生冲突的单元d[i], 加上 1²、 2²等。即d[i] + 1²，d[i] + 2², d[i] + 3²...直到找到空闲单元。
   - **双散列函数探查法**，使用两个散列函数hl和h2，第二个散列函数是用来产生一个固定的增量
     - 第二个散列函数要仔细选择，需满足条件
       （1）排除散列值是0的情况
       （2）产生的散列值必须与表长M互素

<table><tr><td bgcolor=gray><big>链地址法（链表法）</big></td></tr></table>

1. 思路
   - 将哈希值相同的元素构成一个同义词的单链表，并将单链表的头指针存放在哈希表的第i个单元中，查找、插入和删除主要在同义词链表中进行。适用于经常进行插入和删除的情况。
2. 相比开放地址法的优点
   - 处理冲突简单，且无堆积现象，即非同义词决不会发生冲突，因此平均查找长度较短；
   - 各链表上的结点空间是动态申请的，故它更适合于造表前无法确定表长的情况；
   - 删除结点的操作易于实现。
   - 开放定址法为减少冲突，要求装填因子α较小，故当结点规模较大时会浪费很多空间。结点较大时，拉链法中增加的指针域可忽略不计，因此可以认为比较节省空间。
3. 缺点
   - 指针需要额外的空间，故当结点规模较小时，开放定址法较为节省空间，而若将节省的指针空间用来扩大散列表的规模，可使装填因子变小，这又减少了开放定址法中的冲突，从而提高平均查找速度。

<table><tr><td bgcolor=gray><big>再哈希法</big></td></tr></table>

1. 思路
   - 同时构造多个不同的哈希函数，直到冲突不再产生。

<table><tr><td bgcolor=gray><big>建立公共溢出区</big></td></tr></table>
